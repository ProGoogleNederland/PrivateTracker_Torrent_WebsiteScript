...
if ($action == "verwijderen_totaal")
	...
	$res = mysqli_query($con_link, "SELECT * FROM massa_bericht_mods WHERE id=$id") or sqlerr(__FILE__, __LINE__);
	...
	if ($row)
		...
		mysqli_query($con_link, "DELETE FROM massa_bericht_mods WHERE id LIKE $id") or sqlerr(__FILE__, __LINE__);
		}
...
if ($action == "verwijderen")
	...
	if ($id)
		{
		mysqli_query($con_link, "UPDATE massa_bericht_mods set done='yes' WHERE id LIKE $id") or sqlerr(__FILE__, __LINE__);
		}
	header("Refresh: 0; url=". $PHP_SELF);
	die;
	}
...
$aantal = get_row_count("massa_bericht_mods", "WHERE done='no'");

if ($aantal < 1)
	site_error_message("Foutmelding", "Geen nieuwe moderator massa berichten gevonden.");

site_header("Massa berichten moderators");
page_start(98);
tabel_top("Overzicht massa berichten moderators - ".$aantal." stuk(s)","center");
tabel_start(98);

if ($action == "allemaal")
	$res = mysqli_query($con_link, "SELECT * FROM massa_bericht_mods WHERE done='no' ORDER BY added DESC") or sqlerr(__FILE__, __LINE__);	
else
	$res = mysqli_query($con_link, "SELECT * FROM massa_bericht_mods WHERE done='no' ORDER BY added DESC LIMIT 1") or sqlerr(__FILE__, __LINE__);	
while ($row = mysqli_fetch_assoc($res))
	...
	print "<tr>";
	print "<td class=embedded><font color=$font_color><b>";
	print "Naam verzender: <a class=altlink_blue href='userdetails.php?id=".$row['sender']."'>" . @get_username($row['user_id_sender']) . "</a>";
	print "</td>";
	print "<td class=embedded><font color=$font_color><b>";
	print "</td>";
	print "<td class=embedded><font color=$font_color><b>";

...
*/
/*
$res = mysqli_query($con_link, "SELECT users.id AS test, downloaded.torrent as torrent_id FROM users LEFT JOIN downloaded ON users.id = downloaded.user WHERE users.username LIKE 'a%' AND downloaded.downloaded > (1024*1024*1024) AND (downloaded.downloaded / downloaded.uploaded) < 0.8 AND users.warned = 'no' ORDER BY users.id LIMIT 2500") or sqlerr(__FILE__, __LINE__);
//$res = mysqli_query($con_link, "SELECT users.id AS test, downloaded.torrent as hmmz FROM users, downloaded, downup WHERE users.username = downup.user AND downup.downloaded > downup.uploaded AND downloaded.torrent = downup.torrent LIMIT 20") or sqlerr(__FILE__, __LINE__);
while ($row = mysqli_fetch_assoc($res))
	{
	print " - ";
	print get_username($row['test']);
	print " - ";
	print $row['pfff'];
	print "<br>";	
	$tel += 1;
...
//$my_querie = "WHERE username LIKE '".$letter."%' AND downloaded > $download_limiet AND (uploaded / downloaded) < $ratio_limiet AND added < FROM_UNIXTIME($added_limiet) AND class < 2 AND warned != 'yes'";
...
$res = mysqli_query($con_link, "SELECT users.uploaded AS uploaded, users.downloaded AS downloaded, users.added AS added, users.last_access AS last_access, users.id AS user_id, downloaded.torrent as torrent_id FROM users LEFT JOIN downloaded ON users.id = downloaded.user WHERE users.username LIKE 'a%' AND downloaded.downloaded > (1024*1024*1024) AND (downloaded.downloaded / downloaded.uploaded) < 0.8 AND users.warned = 'no' ORDER BY users.id LIMIT 2500") or sqlerr(__FILE__, __LINE__);
...
while ($row = mysqli_fetch_assoc($res))
	...
	print "<tr>";
	print "<td bgcolor=white><a class=altlink_blue href='userdetails.php?id=".$row['id']."' target=_blank>".get_usernamesitesmal($row['user_id'])."</a></td>";
	...
	print "<td bgcolor=white align=center>".get_userratio($row['user_id'])."</td>";
	$waarschuwingen = get_row_count("warnings","WHERE userid=$row[user_id] ORDER by date");
	print "<td bgcolor=white align=center>".$waarschuwingen."</td>";
	print "<td bgcolor=white align=center><INPUT type=\"checkbox\" name=\"messages[]\" value=\"" . $row['user_id'] . "\"></td>";
	print "</tr>";
	}

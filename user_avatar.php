...
if ($mode == "form")
	...
	site_header("Avatar");
	page_start();
	tabel_top("Avatar wijzigen van " . get_username($user_id),"center");
	tabel_start();
	...
	print "<tr><td bgcolor=white class=rowhead>Avatar:</td><td bgcolor=white align=left>";
...
if ($mode == "wijzig")
	...
	mysqli_query($con_link, "UPDATE users SET avatar=$avatar WHERE id=$user_id") or sqlerr(__FILE__, __LINE__);

	site_error_message("Avatar", "Avatar van <a href=userdetails.php?id=" . $user_id . ">" . get_username($user_id) . "</a> is gewijzigd.");
	}
?>